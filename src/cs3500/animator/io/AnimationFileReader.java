...
public class AnimationFileReader {
    ...
    while (sc.hasNext()) {
      ...
      switch (command) {
        case "canvas":
          ...
          break;
        case "slow-motion":
          SlowMInfo slowMInfo = readSlowMInfo(sc);
          builder.addSlowMo(slowMInfo.getSTime(), slowMInfo.getETime(), slowMInfo.getTempo());
          break;
        case "layer":
          LayerInfo layerInfo = readLayerInfo(sc);
          builder.addLayer(layerInfo.getLayerName(), layerInfo.getShapeIDs());
          break;
        case "rectangle":
          ...
          break;
        case "cross":
          RectangleInfo xinfo = readRectangleInfo(sc);
          builder.addCross(
                  xinfo.getName(),
                  xinfo.getX(), xinfo.getY(),
                  xinfo.getWidth(), xinfo.getHeight(),
                  xinfo.getR(), xinfo.getG(), xinfo.getB(),
                  xinfo.getStart(), xinfo.getEnd());
          break;
        case "oval":
  ...
  private SlowMInfo readSlowMInfo(Scanner sc) {
    SlowMInfo info = new SlowMInfo();
    info.setSTime(sc.nextInt());
    info.setETime(sc.nextInt());
    info.setTempo(sc.nextInt());
    return info;
  }

  private LayerInfo readLayerInfo(Scanner sc) {
    LayerInfo info = new LayerInfo();
    info.setLayerName(sc.next());
    String shapeIds = sc.nextLine();
    String[] shapeIdList = shapeIds.split(" ");
    for (String shapeID : shapeIdList) {
      info.addShapeID(shapeID);
    }
    return info;
  }
  ...
  class LayerInfo {
    ...
    private ArrayList<String> shapeIDs;
    ...
    void addShapeID(String shapeID) {
